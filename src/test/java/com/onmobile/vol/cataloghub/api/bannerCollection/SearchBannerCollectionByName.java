...
public class SearchBannerCollectionByName extends RestBaseClass implements Constants {
	...
	public Object[][] getPositiveTestData() {
		...
		Object[][] testData = ExcelConfig.getTestDataAsMap(GET_BANNER_XL_PATH,
				SEARCH_BY_BANNER_COLLECTION_NAME_POSITIVE_SHEET);
		LOGGER.entering(sourceClass, sourceMethod);
		return testData;
	}
	...
	public Object[][] getNegativeTestdata() {
		...
		LOGGER.entering(sourceClass, sourceMethod);
		Object[][] testData = ExcelConfig.getTestDataAsMap(GET_BANNER_XL_PATH,
				SEARCH_BY_BANNER_COLLECTION_NAME_NEGATIVE_SHEET);
		LOGGER.entering(sourceClass, sourceMethod);
		return testData;
	}

	@Test(dataProvider = "getPositiveTestdata")
	public void searchBannerCollectionByname(Map<String, String> testDatList[]) {
		...
		queryParam.put("searchItem", CommonValues.bannerCollectionValues.get("banner_collection_name"));

		Response response = requestGenarator.getRequest(queryParam, pathParam, BASE_URL).when()
				.get(SEARCH_BY_BANNER_COLLECTION_NAME).then().statusCode(Integer.parseInt(testData.get("statusCode"))).and()
				.body("banner_collections.banner_collection_id", hasItem(Integer.parseInt(CommonValues.bannerCollectionValues.get("banner_collection_id"))),
						"banner_collections.banner_collection_name", hasItem(CommonValues.bannerCollectionValues.get("banner_collection_name")))
				.extract().response();

		response.then().assertThat().body(matchesJsonSchemaInClasspath(
				PropertyReader.getProperty(CATALOG_HUB_JSON_SCHEMA_PROPERTY_FILE, SEARCH_BY_BANNER_COLLECTION_NAME_JSON_SCHEMA)));
	...
	public void searchBannerCollectionBynameNegative(Map<String, String> testDatList[]) {
		Map<String, String> testData = testDatList[0];
		while (testData.values().remove(""))
			;
		...
		Response response = requestGenarator.getRequest(queryParam, pathParam, BASE_URL).when()
				.get(SEARCH_BY_BANNER_COLLECTION_NAME).then().statusCode(Integer.parseInt(testData.get("statusCode")))
				.extract().response();
